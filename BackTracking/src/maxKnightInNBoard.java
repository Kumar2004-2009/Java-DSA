public class maxKnightInNBoard {
    ...
    public static void nKnight(char[][] boards,int row,int col,int num) {
        int n = boards.length;
        if(row==n){
//            for(int i=0;i<n;i++){
//                for(int j=0;j<n;j++){
...
//            System.out.println();
        ...
        else if (isSafe(boards, row, col)) {
            boards[row][col] = 'K';
            if (col != n - 1) nKnight(boards, row, col + 1, num + 1);
            else nKnight(boards, row + 1, 0, num + 1);
            boards[row][col] = 'x';
        }
        if (col != n - 1) nKnight(boards, row, col + 1, num + 1);
        else nKnight(boards, row + 1, 0, num + 1);
